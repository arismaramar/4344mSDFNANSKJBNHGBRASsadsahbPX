...
Name=$(curl -sS https://raw.githubusercontent.com/lunoxximpostor/4344mSDFNANSKJBNHGBRASsadsahbPX/main/access | grep $MYIP | awk '{print $2}')
...
PERMISSION () {
    MYIP=$(curl -sS ipv4.icanhazip.com)
    IZIN=$(curl -sS https://raw.githubusercontent.com/lunoxximpostor/4344mSDFNANSKJBNHGBRASsadsahbPX/main/access | awk '{print $4}' | grep $MYIP)
    if [ "$MYIP" = "$IZIN" ]; then
    Bloman
    else
